...
worker(const std::vector<std::pair<std::string, int>> &addresses) {
...
get_ip_port(const std::string &packet) {
  std::string p =
      packet.substr(sizeof("\xFF\xFF\xFF\xFFgetserversResponse") - 1);
...
std::vector<std::string> getUdpServers() {
  ...
  socklen_t server_addr_len = sizeof(server_addr);
  int n = recvfrom(sockfd, buffer, BUFFER_SIZE, 0, (sockaddr *)&server_addr,
                   &server_addr_len);
  if (n < 0) {
    perror("Receive failed");
    close(sockfd);
...
int main() {
  ...
  std::cout << "Server is running on http://localhost:" << LISTEN_PORT
            << std::endl;
